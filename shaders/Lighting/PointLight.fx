...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
    ...
    float4 worldPosition = mul(float4(input.Position,1), VX_MATRIX_WORLD);
...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	float4 colorMapSample = tex2D(colorSampler, texCoord);
    ...
    //get specular power
	float specularPower = colorMapSample.b;// normalData.a * 255;
    //get specular intensity from the colorMap
	float specularIntensity = colorMapSample.a;// tex2D(colorSampler, texCoord).a;
